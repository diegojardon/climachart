...
function formatEpw(epwCSV) {
	...
	var la = d3.csv.parseRows(epwCSV)[0];
	...
	var w = d3.csv.parseRows(epwCSV, function(d, i) {
		if (i > 7) {
			return {
				hour: i-8,
...
function psychrometricChart() {
	...
	var dbAxis = d3.svg.axis().scale(dbScale);
	var hrAxis = d3.svg.axis().scale(hrScale).orient("right");
	...
	function chart(selection) {
		selection.each(function(data, i) {
			...
			/*
			 * DAJR -- Settear valores iniciales de configuracion para la grafica
			 */
			...
			*/
			...
			psychChart.select("#dbAxis")
          	...
          	station.each(function(d, i) {
          		...
          		.attr("text-anchor", "middle")
          		.text(  ((d.city.length > 20) ? (d.city.substring(0, 9) + "...") : d.city) + " | " + d.country);
